...
To set up a proxy using `vite` and `nginx` see [Setting Up a Proxy Pass](docs/getting-started/proxy-pass/setting-up-a-proxy-pass.md).

To set up the package to work without a proxy pass or to work with a proxy pass different from `/api` see [Not Using a Proxy Pass](docs/getting-started/proxy-pass/not-using-a-proxy-pass.md).
